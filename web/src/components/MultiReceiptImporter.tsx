...
export function MultiReceiptImporter() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      showGlobalError({
        title: "Upload Failed",
        message: err.message || "Failed to upload receipts",
      });
    } finally {
      setUploading(false);
    }
    ...
    <div className="card space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--ic-gray-600)] mt-1">
          Import CSV files with multiple receipts per policy. Each row should contain a policy number and up to 10 receipt records (REC_DATE_1-10, REC_NUMBER-10, REC_AMT_1-10).
        </p>
      </div>

      <div className="space-y-3">
        <div>
          <label htmlFor="multi-receipt-file" className="block text-sm font-medium text-[var(--ic-navy)] mb-2">
            Select CSV File
          </label>
          <input
