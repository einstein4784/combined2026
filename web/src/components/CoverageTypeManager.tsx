    ...
    <div className="card space-y-4">
      <div>
        ...
        <p className="text-sm text-[var(--ic-gray-600)]">
          Add or remove coverage types available when creating policies.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-[var(--ic-gray-200)] bg-[var(--ic-gray-50)]">
        {loading ? (
          ...
          <ul className="divide-y divide-[var(--ic-gray-200)]">
            {items.map((item) => (
              <li key={item._id} className="flex items-center justify-between px-3 py-2">
                <span className="text-sm font-semibold text-[var(--ic-navy)]">{item.name}</span>
                <button className="btn btn-ghost text-sm text-red-600" onClick={() => remove(item._id)}>
                  Delete
                </button>
              </li>
            ))}
